...
class AutoEncoder:
    ...
    def visualize(self):
        ...
        axes[1].fill_between(np.arange(self.in_features), self.X_test.iloc[i], out[i], color='lightcoral')
        axes[1].legend(['Original Data', 'Reconstructed Data'])
        axes[1].set_title('Original Vs Reconstructed Data')
        axes[1].set_xlabel('Features')
        axes[1].set_ylabel('Values')
        fig.savefig(f'exported_data/latent_space_{self.latent_space}_batch_{self.batch_size}/plot{self.latent_space}_bs_{self.batch_size}.png')
